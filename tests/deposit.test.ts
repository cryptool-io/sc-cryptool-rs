...
test("Deposit amount too low", async () => {
  await deployer.callContract({
    ...
    await bob.callContract({
      ...
      esdts: [
        { id: CURRENCY1, amount: CURRENCY1_DEPOSIT_TOO_LOW },
      ],
...
test("Deposit amount too high", async () => {
  await deployer.callContract({
    ...
    await bob.callContract({
      ...
      esdts: [
        { id: CURRENCY1, amount: CURRENCY1_DEPOSIT_AMOUNT * BigInt(3)},
      ],
...
test("Deposit but max deposit threshold would be exceeded", async () => {
  await deployer.callContract({
    ...
    }).assertFail({code : 4, message: "Max_deposit threshold would be exceeded"}); 
  });  
...
test("Deposit but hard cap threshold would be exceeded", async () => {
  await deployer.callContract({
    ...
    bob = await world.createWallet({
      ...
      kvs: [
        e.kvs.Esdts([
          { id: CURRENCY1, amount: CURRENCY1_DEPOSIT_MAX},
        ]),
      ],
    });
    ...
    carol = await world.createWallet({
      ...
      kvs: [
        e.kvs.Esdts([
          { id: CURRENCY2, amount: CURRENCY2_DEPOSIT_MAX },
        ]),
      ],
    });
...
test("Deposit with incorect deposit increment", async () => {
  await deployer.callContract({
    ...
    await bob.callContract({
      ...
      esdts: [
        { id: CURRENCY1, amount: CURRENCY1_DEPOSIT_INCORECT_AMOUNT },
      ],
    }).assertFail({code : 4, message: "Payment amount is not a multiple of the deposit increment"});
  });
  ...
  await deployer.callContract({
    ...
    assertAccount(await raisePoolContract.getAccount(), {
      ...
      kvs: [
        ...
        e.kvs.Mapper("deposited_currencies", e.Addr(bob)).UnorderedSet([e.Str(CURRENCY1)]),
        e.kvs.Mapper("deposited_amount", e.Addr(bob), e.Str(CURRENCY1)).Value(e.U(CURRENCY1_DEPOSIT_AMOUNT)),
        e.kvs.Mapper("total_amount").Value(e.U(CURRENCY1_DEPOSIT_AMOUNT)),
        e.kvs.Mapper("address_platform_fee", e.Addr(bob), e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * PLATFORM_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("platform_fee", e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * PLATFORM_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("total_platform_fee")
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * PLATFORM_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("address_group_fee", e.Addr(bob), e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * GROUP_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("group_fee", e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * GROUP_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("total_group_fee")
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * GROUP_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("ambassadors").Set(([[1, e.Addr(deployer)]])),
        e.kvs.Mapper("address_ambassador_fee", e.Addr(bob), e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * AMBASSADOR_FEE / MAX_PERCENTAGE)),
        e.kvs.Mapper("ambassador_fee", e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * AMBASSADOR_FEE / MAX_PERCENTAGE)),
        e.kvs.Mapper("referral_ambassador_fee", e.Addr(deployer), e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * AMBASSADOR_FEE / MAX_PERCENTAGE)),
        e.kvs.Mapper("ambassador_currencies", e.Addr(deployer))
...
test("Deposit Currency1, Currency2 with Bob", async () => {
  await deployer.callContract({
    ...
    const denominated_currency2 = BigInt(CURRENCY2_DEPOSIT_AMOUNT) * BigInt(10 ** (18 - DECIMALS2));
    const total_deposited_amount = BigInt(CURRENCY1_DEPOSIT_AMOUNT) + denominated_currency2
    const total_platform_fee = BigInt(CURRENCY1_DEPOSIT_AMOUNT) * BigInt(PLATFORM_FEE1) / BigInt(MAX_PERCENTAGE) + 
        denominated_currency2 * BigInt(PLATFORM_FEE1) / BigInt(MAX_PERCENTAGE)
    const total_group_fee = BigInt(CURRENCY1_DEPOSIT_AMOUNT) * BigInt(GROUP_FEE1) / BigInt(MAX_PERCENTAGE) + 
    ...
    assertAccount(await raisePoolContract.getAccount(), {
      ...
      kvs: [
        ...
        e.kvs.Mapper("raise_pool_enabled").Value(e.Bool(true)),
        e.kvs.Mapper("addresses").Set(([[1, e.Addr(bob)]])),
        e.kvs.Mapper("deposited_currencies", e.Addr(bob)).UnorderedSet([e.Str(CURRENCY1),e.Str(CURRENCY2)]),
        e.kvs.Mapper("deposited_amount", e.Addr(bob), e.Str(CURRENCY1)).Value(e.U(CURRENCY1_DEPOSIT_AMOUNT)),
        e.kvs.Mapper("deposited_amount", e.Addr(bob), e.Str(CURRENCY2)).Value(e.U(CURRENCY2_DEPOSIT_AMOUNT)),
        e.kvs.Mapper("total_amount").Value(e.U(total_deposited_amount)),
        e.kvs.Mapper("address_platform_fee", e.Addr(bob), e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * PLATFORM_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("address_platform_fee", e.Addr(bob), e.Str(CURRENCY2))
              .Value(e.U(CURRENCY2_DEPOSIT_AMOUNT * PLATFORM_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("platform_fee", e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * PLATFORM_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("platform_fee", e.Str(CURRENCY2))
              .Value(e.U(CURRENCY2_DEPOSIT_AMOUNT * PLATFORM_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("total_platform_fee")
              .Value(e.U(total_platform_fee)),
        e.kvs.Mapper("address_group_fee", e.Addr(bob), e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * GROUP_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("address_group_fee", e.Addr(bob), e.Str(CURRENCY2))
              .Value(e.U(CURRENCY2_DEPOSIT_AMOUNT * GROUP_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("group_fee", e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * GROUP_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("group_fee", e.Str(CURRENCY2))
              .Value(e.U(CURRENCY2_DEPOSIT_AMOUNT * GROUP_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("total_group_fee")
              .Value(e.U(total_group_fee)),
        e.kvs.Mapper("ambassadors").Set(([[1, e.Addr(deployer)]])),
        e.kvs.Mapper("address_ambassador_fee", e.Addr(bob), e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * AMBASSADOR_FEE / MAX_PERCENTAGE)),
        e.kvs.Mapper("address_ambassador_fee", e.Addr(bob), e.Str(CURRENCY2))
              .Value(e.U(CURRENCY2_DEPOSIT_AMOUNT * AMBASSADOR_FEE / MAX_PERCENTAGE)),
        e.kvs.Mapper("ambassador_fee", e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * AMBASSADOR_FEE / MAX_PERCENTAGE)),
        e.kvs.Mapper("ambassador_fee", e.Str(CURRENCY2))
              .Value(e.U(CURRENCY2_DEPOSIT_AMOUNT * AMBASSADOR_FEE / MAX_PERCENTAGE)),
        e.kvs.Mapper("referral_ambassador_fee", e.Addr(deployer), e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * AMBASSADOR_FEE / MAX_PERCENTAGE)),
        e.kvs.Mapper("referral_ambassador_fee", e.Addr(deployer), e.Str(CURRENCY2))
              .Value(e.U(CURRENCY2_DEPOSIT_AMOUNT * AMBASSADOR_FEE / MAX_PERCENTAGE)),
        e.kvs.Mapper("ambassador_currencies", e.Addr(deployer))
...
test("Deposit Currency1, Currency2 with Bob, Currency3 with Carol", async () => {
  await deployer.callContract({
    ...
    const denominated_currency2 = BigInt(CURRENCY2_DEPOSIT_AMOUNT) * BigInt(10 ** (18 - DECIMALS2));
    const denominated_currency3 = BigInt(CURRENCY3_DEPOSIT_AMOUNT) * BigInt(10 ** (18 - DECIMALS3));
    const total_deposited_amount = BigInt(CURRENCY1_DEPOSIT_AMOUNT) + denominated_currency2 + denominated_currency3
    const total_platform_fee = BigInt(CURRENCY1_DEPOSIT_AMOUNT) * BigInt(PLATFORM_FEE1) / BigInt(MAX_PERCENTAGE) + 
        denominated_currency2 * BigInt(PLATFORM_FEE1) / BigInt(MAX_PERCENTAGE) + 
    ...
    assertAccount(await raisePoolContract.getAccount(), {
      ...
      kvs: [
        ...
        e.kvs.Mapper("platform_fee_wallet").Value(e.Addr(deployer)),
        e.kvs.Mapper("group_fee_wallet").Value(e.Addr(deployer)),
        e.kvs.Mapper("payment_currencies").UnorderedSet([e.Str(CURRENCY1), e.Str(CURRENCY2), e.Str(CURRENCY3)]),
        e.kvs.Mapper("currency_decimals", e.Str(CURRENCY1)).Value(e.U32(DECIMALS1)), 
        e.kvs.Mapper("currency_decimals", e.Str(CURRENCY2)).Value(e.U32(DECIMALS2)), 
        ...
        e.kvs.Mapper("release_state").Value(e.Usize(0)),
        e.kvs.Mapper("raise_pool_enabled").Value(e.Bool(true)),
        e.kvs.Mapper("addresses").Set(([[1, e.Addr(bob)], [2, e.Addr(carol)]])),
        e.kvs.Mapper("deposited_currencies", e.Addr(bob)).UnorderedSet([e.Str(CURRENCY1),e.Str(CURRENCY2)]),
        e.kvs.Mapper("deposited_currencies", e.Addr(carol)).UnorderedSet([e.Str(CURRENCY3)]),
        e.kvs.Mapper("deposited_amount", e.Addr(bob), e.Str(CURRENCY1)).Value(e.U(CURRENCY1_DEPOSIT_AMOUNT)),
        e.kvs.Mapper("deposited_amount", e.Addr(bob), e.Str(CURRENCY2)).Value(e.U(CURRENCY2_DEPOSIT_AMOUNT)),
        e.kvs.Mapper("deposited_amount", e.Addr(carol), e.Str(CURRENCY3)).Value(e.U(CURRENCY3_DEPOSIT_AMOUNT)),
        e.kvs.Mapper("total_amount").Value(e.U(total_deposited_amount)),
        e.kvs.Mapper("address_platform_fee", e.Addr(bob), e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * PLATFORM_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("address_platform_fee", e.Addr(bob), e.Str(CURRENCY2))
              .Value(e.U(CURRENCY2_DEPOSIT_AMOUNT * PLATFORM_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("address_platform_fee", e.Addr(carol), e.Str(CURRENCY3))
              .Value(e.U(CURRENCY3_DEPOSIT_AMOUNT * PLATFORM_FEE2 / MAX_PERCENTAGE)),
        e.kvs.Mapper("platform_fee", e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * PLATFORM_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("platform_fee", e.Str(CURRENCY2))
              .Value(e.U(CURRENCY2_DEPOSIT_AMOUNT * PLATFORM_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("platform_fee", e.Str(CURRENCY3))
              .Value(e.U(CURRENCY3_DEPOSIT_AMOUNT * PLATFORM_FEE2 / MAX_PERCENTAGE)),
        e.kvs.Mapper("total_platform_fee")
              .Value(e.U(total_platform_fee)),
        e.kvs.Mapper("address_group_fee", e.Addr(bob), e.Str(CURRENCY1))
        ...
        e.kvs.Mapper("address_group_fee", e.Addr(bob), e.Str(CURRENCY2))
              .Value(e.U(CURRENCY2_DEPOSIT_AMOUNT * GROUP_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("address_group_fee", e.Addr(carol), e.Str(CURRENCY3))
              .Value(e.U(CURRENCY3_DEPOSIT_AMOUNT * GROUP_FEE2 / MAX_PERCENTAGE)),
        e.kvs.Mapper("group_fee", e.Str(CURRENCY1))
              .Value(e.U(CURRENCY1_DEPOSIT_AMOUNT * GROUP_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("group_fee", e.Str(CURRENCY2))
              .Value(e.U(CURRENCY2_DEPOSIT_AMOUNT * GROUP_FEE1 / MAX_PERCENTAGE)),
        e.kvs.Mapper("group_fee", e.Str(CURRENCY3))
              .Value(e.U(CURRENCY3_DEPOSIT_AMOUNT * GROUP_FEE2 / MAX_PERCENTAGE)),
        e.kvs.Mapper("total_group_fee")
              .Value(e.U(total_group_fee)),
        e.kvs.Mapper("ambassadors").Set(([[1, e.Addr(deployer)]])),
