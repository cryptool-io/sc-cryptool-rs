...
const signerPemCarol = fs.readFileSync(path.resolve(process.cwd(), 'tests/wallets/carol.pem')).toString();
const privateKeyCarol = UserSecretKey.fromPem(signerPemCarol);
export const carolAddress = privateKeyCarol.generatePublicKey().toAddress().pubkey();
...
export const SIGNATURE_CAROL_WITHOUT_AMBASSADOR = privateKeyDeployer.sign(DATA_CAROL_WITHOUT_AMBASSADOR);
